...
使用 AI 多模态模型（如 MiMo-Omni）对游戏各场景截图进行自动化视觉审查，发现角色融合度、光影一致性、UI 适配等问题，然后针对性修复。
...
2. 浏览器加载游戏，逐场景截图
   - 使用 goScene() 跳转: apartment → street → bar → alley → tower → server → rooftop → underground → core
   - 每个场景需先关闭对话框再截图（避免 UI 遮挡）
   - 保存到 screenshots/ 目录，命名: 01_apartment.png, 02_street.png ...

3. 多模态模型分析截图
   bash mimo_api.sh image screenshots/01_apartment.png "分析提示词..."
...
每个场景按以下维度检查，标注严重程度（严重/中等/轻微）：

| 维度 | 检查内容 |
|------|----------|
| 角色比例 | 角色与场景家具/建筑的尺寸是否合理 |
| 光影一致性 | 角色是否受场景光源影响、阴影方向是否统一 |
| 角色融合度 | 角色像素风格是否与背景融合、有无环境色反射 |
| 物品摆放 | 场景物品是否有物理逻辑、有无穿模/悬浮 |
| UI 适配 | 对话框/物品栏是否与场景色调协调 |
| 构图 | 视觉焦点是否清晰、有无引导线 |
| 像素瑕疵 | 有无锯齿、渲染异常、AI 生成伪影 |
| 色彩统一 | 整体色调是否一致 |
...
| 问题 | 修复方式 |
|------|----------|
| 角色"贴纸感" | 增加环境光映射、阴影、倒影 |
| LoadingTracker 卡住 | 确保 loaded ≤ total，后台资产计入总数 |
| 光影方向不一致 | 统一场景主光源方向，角色受光面适配 |
| UI 颜色割裂 | 对话框边框颜色随场景主色调变化 |
| 角色比例失调 | 调整 SCENES 中 character.scale 参数 |

### 注意事项

- 截图前必须关闭对话框（点击 canvas 或等对话结束）
- 游戏 LoadingTracker 可能因 loaded > total 卡住，需手动 bypass 或确保代码修复
- headless 浏览器可能不支持 WebGL（光照引擎会 fallback 到预生成帧）
- 每次修复后需重新截图验证
...
| tower | underground | (0.50, 0.80) | 从地下上来 → 中央 |
| server | tower | (0.20, 0.85) | 从塔楼进来 → 左侧入口 |
| server | rooftop | (0.80, 0.85) | 从楼顶下来 → 右侧通道口 |
| rooftop | server | (0.50, 0.80) | 从服务器室上来 → 中央 |
| underground | tower | (0.50, 0.80) | 从塔楼进来 → 中央入口 |
| core | underground | (0.50, 0.80) | 从地下实验室进来 → 中央 |
