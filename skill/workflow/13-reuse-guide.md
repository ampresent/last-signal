...
1. **写剧情大纲** — 列出场景/物品/角色
2. **生成基础素材** — 修改 `scripts/gen_assets.py` 中的 PROMPTS 和 PORTRAITS
3. **配置光源** — 在 `scripts/gen_depth_lighting.py` 的 SCENE_LIGHTS 中添加光源配置
4. **配置交互** — 在 `scripts/gen_masks.py` 的 SCENES 中定义物体 bbox、walkable、water
...
只需要：
- 1 个场景 prompt（scripts/gen_assets.py）
- 1 个光源配置（scripts/gen_depth_lighting.py）
