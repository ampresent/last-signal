...
## 12. 迭代地面检测 — patch 太大导致误覆盖

**现象**：SAM mask 包含墙壁、家具等非地面区域
**原因**：Omni 返回的 bbox 太大，覆盖了非地面物体
**解决**：使用 ≤50px 小 patch + 逐块审查（每个 patch 单独 Omni PASS/FAIL，FAIL 直接丢弃）
**代码**：`gen_ground_mask.py` v2 — `MAX_PATCH_SIZE=50`，每轮 4 个 patch

## 13. 迭代地面检测 — patch 空间分布不均

**现象**：所有 patch 都集中在底部（近景），遗漏远处地面
**原因**：Omni 倾向于选择最明显的地面区域
**解决**：Prompt 强调"分散在不同位置"，下一轮参考已有 mask 识别遗漏区域

## 14. MobileSAM v2 模型不存在

**现象**：`FileNotFoundError` 或 15-byte "Entry not found" 文件
**原因**：hf-mirror.com 上的 v2 模型文件不存在
**解决**：使用 v1 模型（从 R2 下载）
